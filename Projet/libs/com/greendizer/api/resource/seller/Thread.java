...
/**
 * Represents a thread from the seller perspective.
 */
public class Thread extends AbstractThread<Message> {

  /**
   * Creates a new thread with the API client, its parent URI, the recipient buyer URI, its subject and its message.
   * @param client API client.
   * @param parentURI Parent URI.
   * @param buyerURI Recipient seller URI.
   * @param subject Thread subject.
   * @param message Thread message text.
   */
  public Thread(AbstractClient<?, ?, ?> client, String parentURI, String buyerURI, String subject, String message) {
    this(client, parentURI);
    setProperty("recipient", buyerURI);
  ...
  /**
   * Returns the thread buyer company.
   * @return The thread buyer company.
   */
